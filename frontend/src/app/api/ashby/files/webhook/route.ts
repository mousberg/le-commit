// Internal webhook endpoint for CV downloads from database triggers
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Use the existing download logic from the main files endpoint
    // Import and call the download function
    ...
    return NextResponse.json({
      success: true,
      message: 'Resume successfully processed via webhook',
      fileName,
      fileSize: fileBuffer.byteLength
    });

  } catch (error) {
    console.error('Webhook CV processing error:', error);
    return NextResponse.json(
      { error: 'Failed to process CV webhook', success: false },
      { status: 500 }
    );
  }
}